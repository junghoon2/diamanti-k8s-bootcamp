...
- ### Kubernetes 환경에서 외부 NAS 서버의 Volume을 POD Volume으로 사용 가능
- ### NAS 서버 -> PV -> PVC -> POD Volume Mount 순서
...
소스 코드 : [PV](./nfs-pv.yml) [PVC](./nfs-pvc.yml)
```
vi nfs-pv.yml
...
## Deploy with NFS-PVC
소스 코드 : [NFS-PVC-Deply](./nfs-pvc-deploy.yml)

```
vi nfs-pvc-deploy.yml

기존 PVC 설정과 동일하게 PVC 이름만 지정하면 됨
(...)
        volumeMounts:
        - name: nfs-vol
          mountPath: /nfs
